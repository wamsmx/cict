...
url = "http://localhost:6152/historian/timeseriesdata/read/current/650,11,63,205,85,58,53,635,260,250,286/json"
...
while True:
    ...
    for i in range(len(angle0)):
        ...
        _angle1 = angle1[i]
        _unwrapped = (
            _angle1 - round(((_angle1 - _angle0) / 2 / math.pi), 0) * 2 * math.pi
        )
    ...
    for i in range(len(relative_angle)):
        _self_norm_angle = round(
            (relative_angle[i] - relative_angle_0[i]) * 180 / math.pi, 1
        )
